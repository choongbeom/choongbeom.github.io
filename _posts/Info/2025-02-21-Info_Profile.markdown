---
layout: post
title: Profile
date: 2025-02-21 11:30 +0900
category: INFO
---

### 16년 차 개발자, 다양한 경험과 유연한 사고를 가진 김충범입니다.
    
|Contact  | Info |
|----------|--------------------------------|
| **Email** | toonme1@gmail.com or toonme@nate.com |
| **Messenger** | KakaoTalk (toonme) |


| Channel  | Link |
|----------|--------------------------------|
| **Blog**  | [choongbeom.github.io](https://choongbeom.github.io/) |
| **GitHub** | [github.com/choongbeom](https://github.com/choongbeom/) |

---

### 📢 **<span style="color:gray">Introduce</span>**

**자동화 기기 및 본인확인 시스템 전문가**로, 16년간 개발 및 기획을 수행해 왔습니다.  
금융권 및 공공기관을 대상으로 **본인확인 솔루션, 스캐너 시스템, 자동화 기기** 개발을 주로 담당했습니다.  
스타트업 및 웹솔루션 개발 경험을 보유하고 있으며, **기획부터 개발, 유지보수까지 전 과정**을 수행해 왔습니다.

---

### 🚀 **<span style="color:gray">Work Experience</span>**  

### 🏢 [보임테크놀러지](https://voimtech.com/)  
본인인증, 신분증스캐너, 스캐너 솔루션 전문업체  

#### 📌 Ctrl+R팀 팀장 (2025.01 - 현재)  
- **AI바우처 지원사업**  
- **자사 파트너 몰**기획 및 MVP 개발 
  - ERP 데이터 분석 기반 사업화 기획

#### 📌 AI제품개발팀 팀장 (2024.01 - 2024.12)
- **무인주차 할인 간소화 솔루션** 기획 및 MVP 개발  
  - 본인확인 및 주차 요금 감면 절차 자동화  
  - QR 코드 기반 사용자 인증 시스템 설계  

#### 📌 개발파트 파트장 (2023.01 - 2023.12)
- **선거관리위원회 신형 사전투표 본인확인장비** 소프트웨어 개발(2024년 상반기 완료) 
- 기존 본인확인기 ActiveX 제거 및 Local Server 방식 전환(2024년 상반기 완료)  

#### 📌 솔루션개발팀 매니저 (2022.01 - 2022.12)
- **ESG 웹솔루션 개발** (환경, 사회, 거버넌스 관련 플랫폼)  
- **특허 관련 서비스 플랫폼 기획**  
- **아우디/벤틀리 본인확인 인증 시스템 유지보수**  

#### 📌 클라우드개발팀 매니저 (2021.07 - 2022.01)
- **스타트업([라이트브라더스](https://wrightbrothers.kr/)) 웹사이트 개발**  
- 신규 서비스 개발 (시세조회 API 및 Admin 관리 시스템 - 미오픈)  
- 상품 구매 UX 개선 (무한스크롤 뒤로가기 이슈 해결)  

#### 📌 시스템개발팀 책임연구원 (2014.10 - 2021.12)
- **TWAIN 기반 스캐너 인터페이스 SDK 개발** (20개 이상 고객사 적용)  
- 금융권 및 공공기관 스캐너 시스템 연동 (80개 이상 고객사)  
- **선거관리위원회 사전투표 본인확인기 및 선거관련 프로그램 개발**  
- 본인확인 및 자동화 시스템 개발 및 유지보수  

### 🏢 [네오아이씨피](http://www.neotk.com/html/main.php/)  
CD, ATM, 정산기관련 자동화기기 개발업체  

#### 📌 소프트웨어팀 주임연구원 (2009.07 - 2013.11)
- **국내 및 해외 CD/ATM/정산기 신규 개발 및 유지보수**  
- **자동화 기기 미들웨어 개발** (기기 시스템과 모듈 연동)  
- 금융권 및 공공기관 자동화 기기 소프트웨어 유지보수  

---

### 🚀 **<span style="color:gray">Other Experience</span>**

### 🎮 3인 개발팀 운영 (2013.12 - 2014.09)  
Unity3D 기반 **안드로이드 게임 2종 출시 및 운영**  
- 게임 디자인, 개발, 퍼블리싱까지 전 과정 수행  
- Google Play 스토어 런칭

#### 📌 **망치쾅쾅쾅** ([다운로드 링크](https://apkcombo.com/ko/mangchi-kwangkwangkwang/com.b2hsoft.HammerBBBPaid/))  
- 현재 운영 종료  

#### 📌 **산수의 모험** ([다운로드 링크](https://apkcombo.com/ko/the-adventures-of-arithmetic/com.b2hsoft.taoa/))  
- 현재 운영 종료  

---

### 💻 **<span style="color:gray">Tech Stack</span>**  
#### ✅ **현재 주력 기술**
- **Windows Application:** C/C++, MFC, C#(Winform)  
- **Back-End:** Java, Spring Boot, AWS  
- **Front-End:** Nuxt.js, Vue.js  
- **Database:** MariaDB, MySQL  
- **ETC:** Git/GitHub  

#### ⏳ **과거 경험**
- **Linux Application:** C/C++, Qt  
- **Android Application:** Unity3D, C#  

### **<span style="color:gray">Education 및 Career Description</span>**

### 🎓 **Education**  
- **부산동의대학교 멀티미디어공학과** (2003.03 - 2010.02)  
- **논문:** [적외선 조명 카메라를 이용한 동공 마우스 시스템](https://scienceon.kisti.re.kr/srch/selectPORSrchArticle.do?cn=NPAP09751794&dbt=NPAP)  

### 📄 **Career Description**  
📌 **세부 경력 및 프로젝트 상세 내용은 아래 문서를 참고하세요.**  
📌 [경력기술서 보기 (PDF)](https://docs.google.com/viewer?url=https://github.com/choongbeom/profile/blob/master/careerdescription.pdf?raw=True)  
